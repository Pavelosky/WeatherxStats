...
void WeatherFrame::addTemperature(double temp) {
    ...
    if (temp > highTemp) { // Check if the temperature is higher than the current highest temperature
        highTemp = temp; // If so, update the highest temperature
    }

    if (temp < lowTemp) { // Check if the temperature is lower than the current lowest temperature
